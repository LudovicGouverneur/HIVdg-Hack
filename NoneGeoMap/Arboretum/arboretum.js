var w = 680, h = w, p = 10, max = 100, nodes = [{ x: w / 2, y: h / 2, size: 1 }], links = [];
...
var link = vis.selectAll("line.link"), node = vis.selectAll("circle.node");
d3.timer(function () {
    var source = nodes[~~(Math.random() * nodes.length)], bud = { x: source.x + Math.random() - .5, y: source.y + Math.random() - .5, parent: source, size: 1 };
    inflate(bud);
